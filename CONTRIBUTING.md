...
If you want to manually build and publish the `develop` branches of TwitterServer's 
...
from the `develop` branch of TwitterServer's other Twitter open source dependencies.
...
## Building TwitterServer

TwitterServer is built using [sbt][sbt]. When building please use the included
[`./sbt`](https://github.com/twitter/twitter-server/blob/develop/sbt) script which
...
1.  From your branch, make a pull request against `twitter/twitter-server/develop`
1.  Work with repo maintainers to get your change reviewed
1.  Wait for your change to be pulled into `twitter/twitter-server/develop`
1.  Merge `twitter/twitter-server/develop` into your origin `develop`
...
When creating an issue please try to ahere to the following format:
    ...
    twitter-server: One line summary of the issue (less than 72 characters)
...
80 columns (You can use `fmt -n -p -w 80` to accomplish this).
    ...
    twitter-server: One line description of your change (less than 72 characters)
...
The twitter-server repository on GitHub is kept in sync with an internal repository at
Twitter. For the most part this process should be transparent to users,
...
We also welcome improvements to the TwitterServer documentation, which is maintained
in this repository and hosted on [the corresponding GitHub Pages site][3].

TwitterServer uses [Sphinx][4] to generate its user guide via the built-in Sphinx
...
[3]: https://twitter.github.io/twitter-server/
